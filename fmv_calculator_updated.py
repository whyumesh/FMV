...
def create_dvl_updated_export(fmv_df: pd.DataFrame) -> pd.DataFrame:
    """
    Create DVL_updated.xlsx export with tier, specialty, email, and DVL code
    
    Args:
        fmv_df: FMV Calculator DataFrame with calculated tiers
    
    Returns:
        DataFrame ready for DVL_updated.xlsx export
    """
    logger.info("Creating DVL_updated export data...")
    ...
    dvl_updated_df = pd.DataFrame()
    
    for fmv_col, export_col in export_columns.items():
        if fmv_col in fmv_df.columns:
            dvl_updated_df[export_col] = fmv_df[fmv_col]
        else:
            logger.warning(f"Column '{fmv_col}' not found in FMV Calculator")
            dvl_updated_df[export_col] = None
    
    # Remove rows where essential data is missing
    initial_count = len(dvl_updated_df)
    
    # Keep only rows with valid email and DVL code
    dvl_updated_df = dvl_updated_df[
        (dvl_updated_df['HCP Email'].notna()) & 
        (dvl_updated_df['HCP Email'] != '') &
        (dvl_updated_df['DVL Code'].notna()) &
        (dvl_updated_df['DVL Code'] != '')
    ]
    
    removed_count = initial_count - len(dvl_updated_df)
    if removed_count > 0:
        logger.info(f"Removed {removed_count} rows with missing essential data from DVL export")
    ...
    tier_counts = dvl_updated_df['Tier'].value_counts()
    logger.info("Tier distribution in DVL export:")
    for tier, count in tier_counts.items():
        logger.info(f"   - {tier}: {count} doctors")
    
    return dvl_updated_df

def save_results(fmv_df: pd.DataFrame, missing_df: pd.DataFrame, dvl_updated_df: Optional[pd.DataFrame] = None) -> bool:
    """
    Save all results to files including the new DVL_updated.xlsx
    
    Args:
        fmv_df: Updated FMV Calculator DataFrame
        missing_df: Missing doctors DataFrame
        dvl_updated_df: DVL updated export DataFrame
    ...
    try:
        # Save DVL_updated.xlsx
        if dvl_updated_df is not None and not dvl_updated_df.empty:
            dvl_updated_df.to_excel(DVL_UPDATED_FILE, index=False, engine='openpyxl', sheet_name='DVL Updated')
            logger.info(f"DVL_updated.xlsx saved: {len(dvl_updated_df)} records")
        ...
        logger.error(f"Failed to save DVL_updated.xlsx: {str(e)}")
        success = False
...
def main():
    ...
    try:
        ...
        # Create DVL_updated export
        dvl_updated_df = create_dvl_updated_export(updated_fmv)
        
        # Save results including DVL_updated.xlsx
        if save_results(updated_fmv, missing_df, dvl_updated_df):
            logger.info("=" * 60)
            ...
            logger.info(f"   - DVL_updated records: {len(dvl_updated_df) if dvl_updated_df is not None else 0}")
            logger.info(f"   - Backup created: {BACKUP_FILE}")
            logger.info(f"   - DVL_updated file: {DVL_UPDATED_FILE}")
        else:
