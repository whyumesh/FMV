...
BACKUP_FILE = os.path.join(FOLDER_PATH, f"FMV_Calculator_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv")
...
def safe_read_csv(file_path: str, usecols: Optional[List[str]] = None, required: bool = True) -> pd.DataFrame:
    """
    Safely read CSV file with multiple encoding attempts and error handling
...
def load_data() -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    fmv_df = safe_read_csv(FMV_FILE, required=True)
    ...
    cvdump_df = safe_read_csv(CVDUMP_FILE, usecols=CVDUMP_COLUMNS, required=True)
    
    # Load DVL data
    logger.info("Loading DVL data...")
    dvl_df = safe_read_csv(DVL_FILE, required=True)
...
def update_fmv_calculator(fmv_df: pd.DataFrame, matched_df: pd.DataFrame) -> pd.DataFrame:
    """
    Update FMV Calculator with new matched data
    ...
    if new_data.empty:
        logger.info("All matched doctors already exist in FMV Calculator")
        return fmv_df
...
def save_results(fmv_df: pd.DataFrame, missing_df: pd.DataFrame) -> bool:
    ...
    try:
        ...
        logger.info(f"FMV Calculator saved: {len(fmv_df)} total records")
